...
const WalletModal = ({ isOpen, onClose }) => {
  ...
  const wallets = [
    { name: "MetaMask", connector: injected(), logo: "/wallet-logos/metamask-logo.svg" },
    { name: "WalletConnect", connector: walletConnect({ projectId }), logo: "/wallet-logos/walletconnect-logo.svg" },
    { name: "Coinbase Wallet", connector: coinbaseWallet({ appName }), logo: "/wallet-logos/coinbase-logo.svg" },
  ];
          ...
          {wallets.map(({ name, connector, logo }) => (
            ...
            >
              <div className="flex items-center">
                ...
                <span>{connectingWallet === name ? "Connecting..." : name}</span>
              </div>
              {connectingWallet === name && <Loader2 size={20} className="animate-spin" />}
            </button>
          ))}
        </div>
